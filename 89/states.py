...
def get_every_nth_state(states=states, n=10):
    ...
    return [states[x] for x in range(n-1, len(states), n)]
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    return list(sorted(us_state_abbrev.values()))[0:10] + sorted(states)[-10:]
    # Solution response: return sorted(us_state_abbrev.values())[:10] + sorted(states)[-10:]

print (get_every_nth_state())
print (get_every_nth_state(n=20))
print(get_state_abbrev('Illinois')) # 'IL'
print(get_state_abbrev('North Dakota')) # 'ND'
print(get_state_abbrev('bogus')) # NOT_FOUND
