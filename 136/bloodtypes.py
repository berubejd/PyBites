...
def check_bt(donor, recipient):
    ...
    table = {
        ...
        5: [0, 1, 4, 5],  # A+
        6: [0, 2, 4, 6],  # AB-
        7: [0, 1, 2, 3, 4, 5, 6, 7],  # AB+
    }

    return validate_bt_input(donor).value in table[validate_bt_input(recipient).value]


def validate_bt_input(bt):
    if not type(bt) in [str, int, Bloodtype]:
        raise TypeError

    if type(bt) is str:
        try:
            bt = blood_type_text[bt.upper()]
        except:
            raise ValueError
